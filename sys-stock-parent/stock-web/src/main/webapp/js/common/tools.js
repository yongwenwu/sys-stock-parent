 ...
 */
var tools = {	/***************************************************************
				 * 请求后台交互，获取JSON对象 url : 请求地址 param: 传值参数 async:
				 * 发送方式，同步（false）或异步（true） callback：响应处理函数，执行成功调用返回方法
				 */
	...
	/**
	 * 表单内控件转换为POST请求的JSON格式 form : 表单对象
	 */
	formToJson : function(form, all) {
		var json = {};
		if (!all) {
			$(form).find("input[name][disabled!=disabled]").each(
					function(i, obj) {
						json[$(obj).attr("name")] = $(obj).val();
					});
			$(form).find("input[name][type=hidden][disabled!=disabled]").each(
					function(i, obj) {
						json[$(obj).attr("name")] = $(obj).val();
					});
			$(form).find("textarea[name][disabled!=disabled]").each(
					function(i, obj) {
						json[$(obj).attr("name")] = $(obj).val();
					});
			$(form).find("select[name][disabled!=disabled]").each(
					function(i, obj) {
						json[$(obj).attr("name")] = $(obj).val();
					});
			$(form).find("input[name][type=checkbox][disabled!=disabled]")
					.each(function(i, obj) {
						if ($(obj).attr("checked")) {
			...
			$(form).find("input[name][type=radio][disabled!=disabled]:checked")
					.each(function(i, obj) {
						json[$(obj).attr("name")] = $(obj).val();
					});

			$(form).find("input[name][type=password][disabled!=disabled]")
					.each(function(i, obj) {
						json[$(obj).attr("name")] = $(obj).val();
					});
		} else {
			...
			$(form).find("input[name][type=radio]:checked").each(
					function(i, obj) {
						json[$(obj).attr("name")] = $(obj).val();
					});
	...
	getFileTypeImg : function(ext) {
		return "<img align=\"absMiddle\" src=\"" + systemImagePath
				+ "/filetype/" + ext.toLowerCase() + ".gif"
				+ "\" onerror=\"this.src='" + systemImagePath
				+ "/filetype/defaut.gif'\"/>";
	},
	...
	isMedia : function(ext) {
		ext = ext.toLowerCase();
		if (ext == "flv" || ext == "3gp" || ext == "mov" || ext == "mp4"
				|| ext == "f4v") {
			return true;
		}
		return false;
 ...
 * 把Json数据绑定到控件
 * 
 * json : json对象 filters : 过滤不需要绑定的控件
 */
	...
	for ( var property in json) {
		...
		if (cntrlCnt == 1) {
			...
			if (cntrl.tagName.toLowerCase() == "input"
					&& cntrl.type.toLowerCase() == "checkbox") {
